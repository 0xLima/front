...
declare module "expo-router" {
  ...
  // prettier-ignore
  type StaticRoutes = `/` | `/(auth)/_layout` | `/_layout` | `/(auth)/` | `/(auth)` | `/(auth)/login` | `/login` | `/(tabs)/_layout` | `/(tabs)/` | `/(tabs)` | `/(tabs)/music` | `/music` | `/(tabs)/ticket` | `/ticket` | `/(tabs)/two` | `/two` | `/(tabs)/videos` | `/videos` | `/+html` | `/create` | `/createProfile` | `/modal` | `/profile` | `/successMessage`;
  // prettier-ignore
  type DynamicRoutes<T extends string> = `/${CatchAllRoutePart<T>}` | `/artist/${SingleRoutePart<T>}` | `/vid/${SingleRoutePart<T>}`;
  // prettier-ignore
  type DynamicRouteTemplate = `/[...missing]` | `/artist/[profile]` | `/vid/[title]`;
